    ...
    try:
        ...
        if route != 0:
            ...
            for stop in route:
                ...
                if stop[0] != 0:
                    route_with_titles.append(
                        (stop[0], Brewery.objects.find_by_id(stop[0]).name, round(stop[2], 2), stop[1],
        ...
        print("Error occured at get_greedy_star")
        return 0, 0, 0, 0
...
def greedy_star(home):
    try:
        ...
        if layer == 0:  # Area for searching. From 0 to 450
            ...
            set_area = list(
                sorted(filter(lambda x: x[3] == dir and 350 <= x[2] <= 1000, measured_list), key=itemgetter(1),
                       reverse=True))
            if (len(set_area) < 50): list(
